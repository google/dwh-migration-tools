...
public class InstructionReceiverTest {
  ...
  private void testInstructionReceiver(
      String name, byte[] serverData, byte[] clientData, int blockSize) throws Exception {
    RsyncTestRunner runner =
        new RsyncTestRunner(name, ByteSource.wrap(serverData), ByteSource.wrap(clientData));
  ...
  public void testInstructionReceiverSimple() throws Exception {
    ...
    byte[] clientData =
        new byte[] {0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12, 42, 43, 44, 13, 14, 15, 16};
