...
public class BigQueryClientUtils {
    ...
    @Nonnull
    public static <T> T runWithBackOff(@Nonnull BigQueryCallable<T> callable) throws BigQueryException, IOException, InterruptedException {
        BackOff backoff = new ExponentialBackOff(); // Default max elapsed time is 15 minutes, specified in ExponentialBackOff itself.
        int retryCount = 0;
        for (;;) {
            BigQueryException exception;
            try {
                // For the inner BigQuery call, the max elapsed time is 50 seconds before we re-hit the outer loop.
            ...
            long backOffMillis = backoff.nextBackOffMillis();   // throws IOException for some unknown reason.
