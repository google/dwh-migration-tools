...
public class ExecutorManagerTest {
    ...
    private void test(@Nonnull ExecutorManager manager, int niter) throws Exception {
        ...
        for (int i = 0; i < niter; i++) {
            for (int j = 0; j < n; j++) {
                ...
                manager.submit(new Callable<Object>() {
                    @Override
                ...
                manager.execute(new Callable<Object>() {
                    @Override
                    public Void call() throws Exception {
                        // LOG.info("Start " + name);
                        Thread.sleep(tm);
    ...
    public void testParallelLimited() throws Exception {
        ExecutorService executor = new ThreadPoolExecutor(2, 2, 1, TimeUnit.SECONDS, new ArrayBlockingQueue<>(10), new ThreadPoolExecutor.CallerRunsPolicy());
        ExecutorManager manager = new ExecutorManager(executor);
        test(manager, 2);
        executor.shutdownNow();
    }
