...
public class ConcurrentRecordProgressMonitor extends AbstractConcurrentProgressMonitor {
    ...
    public ConcurrentRecordProgressMonitor(@Nonnull String name, @Nonnegative long total) {
        ...
        this.future = ExecutorHolder.EXECUTOR_SERVICE.scheduleWithFixedDelay(new Update(this, name), DELAY, DELAY, TimeUnit.SECONDS);
    }
    ...
    private void update(@Nonnull String status) {
        ...
        String rate = (elapsed == 0) ? "infinity" : Long.toString((long) (count * 1000L / (double) elapsed));
        if (total > 0) {
            LOG.debug(name + ": " + status + " " + count + " / " + total + " items (" + String.format("%2.2f", 100d * count / total) + "%) in " + stopwatch + " (" + rate + "/sec) " + newMemorySummary());
        } else {
            LOG.debug(name + ": " + status + " " + count + " items in " + stopwatch + " (" + rate + "/sec) " + newMemorySummary());
        }
    }
    ...
    private static class Update implements Runnable {
        ...
        public Update(@UnderInitialization ConcurrentRecordProgressMonitor monitor, @Nonnull String name) {
        ...
        public void run() {
            ...
            if (monitor == null)
                throw new IllegalStateException("Monitor " + name + " garbage-collected without being closed.");
