...
public class ExecutorManager implements AutoCloseable, Executor {
    ...
    @Nonnull
    public static ExecutorService newExecutorServiceWithBackpressure(@Nonnull String name, int nthreads, int qfactor) {
        if (nthreads <= 1)
        ...
        ThreadPoolExecutor executor = new ThreadPoolExecutor(nthreads, nthreads, 30L, TimeUnit.SECONDS, new ArrayBlockingQueue<>(nthreads * qfactor), new ThreadPoolExecutor.CallerRunsPolicy());
    ...
    @Nonnull
    public static ExecutorService newExecutorServiceWithBackpressure(@Nonnull String name, int nthreads) {
    ...
    public static ExecutorService newExecutorServiceWithBackpressure(@Nonnull String name) {
        // We use ncpus - 1 because the backpressure feeds back into the source thread, which will therefore run at 100% CPU as well.
    ...
    public static ExecutorService newUnboundedExecutorService(@Nonnull String name, int nthreads) {
        ThreadPoolExecutor executor = new ThreadPoolExecutor(nthreads, nthreads, 30L, TimeUnit.SECONDS, new LinkedBlockingDeque<>(), new ThreadPoolExecutor.CallerRunsPolicy());
        executor.setThreadFactory(new ThreadFactoryBuilder().setNameFormat(name + "-%d").setDaemon(true).build());
        executor.allowCoreThreadTimeOut(true);
        return executor;
    }
