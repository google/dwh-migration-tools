...
@RespectsInput(order = ConnectorArguments.OPT_PORT_ORDER, arg = ConnectorArguments.OPT_PORT, description = "The port of the server.", required = ConnectorArguments.OPT_REQUIRED_IF_NOT_URL, defaultValue =
    "" + RedshiftMetadataConnector.OPT_PORT_DEFAULT)
public class RedshiftMetadataConnector extends AbstractRedshiftConnector implements
    MetadataConnector, RedshiftMetadataDumpFormat {
  ...
  public void addTasksTo(List<? super Task<?>> out, ConnectorArguments arguments) {
    ...
    // \l
    parallelTask.addTask(new JdbcSelectTask("database.csv",
    ...
    // \dT
    parallelTask.addTask(new JdbcSelectTask("types.csv",
    ...
    // \da
    parallelTask.addTask(new JdbcSelectTask("aggregates.csv",
    ...
    // \dC
    parallelTask.addTask(new JdbcSelectTask("casts.csv",
    ...
    parallelTask.addTask(new JdbcSelectTask(
    ...
    parallelTask.addTask(
        new JdbcSelectTask(RedshiftMetadataDumpFormat.InformationSchemaColumns.ZIP_ENTRY_NAME,
            "select * from information_schema.columns where table_schema not in " + PG_SCHEMAS));
    parallelTask.addTask(
        new JdbcSelectTask(RedshiftMetadataDumpFormat.PgTableDef.ZIP_ENTRY_NAME_SYSTEM,
            "select * from pg_table_def where schemaname in " + PG_SCHEMAS));
    parallelTask.addTask(new JdbcSelectTask(RedshiftMetadataDumpFormat.PgTableDef.ZIP_ENTRY_NAME,
        "select * from pg_table_def where schemaname not in " + PG_SCHEMAS));
    parallelTask.addTask(
        new JdbcSelectTask(RedshiftMetadataDumpFormat.PgViews.ZIP_ENTRY_NAME_SYSTEM,
            "select * from pg_views where schemaname in " + PG_SCHEMAS));
    parallelTask.addTask(new JdbcSelectTask(RedshiftMetadataDumpFormat.PgViews.ZIP_ENTRY_NAME,
