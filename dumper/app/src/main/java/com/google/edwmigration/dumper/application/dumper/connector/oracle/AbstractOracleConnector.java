...
/**
 * https://docs.oracle.com/cd/B28359_01/java.111/b31224/getsta.htm
 ...
 *
 * https://www.oracle.com/technetwork/database/application-development/jdbc-eecloud-3089380.html
 *
 * Recommended Oracle way to dump code:
 * <code>
 ...
 * </code>
 * but this needs to be done by folks who have access to the oracle server given
 * <code>directory={oracle_directory}, dumpfile={dumpfilename}</code> are on the
 * same physical server that Oracle is running on.
 */
@RespectsArgumentDriverRequired
@RespectsArgumentHostUnlessUrl
@RespectsInput(order = ConnectorArguments.OPT_PORT_ORDER, arg = ConnectorArguments.OPT_PORT, description = "The port of the server.", required = ConnectorArguments.OPT_REQUIRED_IF_NOT_URL, defaultValue = "" + AbstractOracleConnector.OPT_PORT_DEFAULT)
@RespectsArgumentUser
@RespectsArgumentPassword
@RespectsArgumentUri
@RespectsInputs({
    // Although RespectsInput is @Repeatable, errorprone fails on it.
    @RespectsInput(order = 450, arg = ConnectorArguments.OPT_ORACLE_SID, description = "The Oracle SID for JDBC onnection"),
    @RespectsInput(order = 500, arg = ConnectorArguments.OPT_ORACLE_SERVICE, description = "The Oracle Service name for JDBC connection")
})
public abstract class AbstractOracleConnector extends AbstractJdbcConnector {
    ...
    private boolean isOracleSid(ConnectorArguments arguments) throws MetadataDumperUsageException {
        ...
        if (sid != null && service == null)
            ...
            throw new MetadataDumperUsageException("Provide either -oracle-service or -oracle-sid for oracle dumper");
    }
