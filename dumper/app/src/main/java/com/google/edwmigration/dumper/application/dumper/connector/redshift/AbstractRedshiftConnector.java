...
@RespectsInput(order = 400, arg = ConnectorArguments.OPT_PASSWORD, description = "The password for the database connection.", required = "If present, forces password authentication")
@RespectsInput(order = 450, arg = ConnectorArguments.OPT_IAM_PROFILE, description = "The IAM Profile to use for authentication.", required = "If present, performs profile-based IAM authentication")
@RespectsInput(order = 455, arg = ConnectorArguments.OPT_IAM_ACCESSKEYID, description = "The IAM Access Key ID to use for authentication.", required = "If present, performs explicit IAM authentication")
@RespectsInput(order = 456, arg = ConnectorArguments.OPT_IAM_SECRETACCESSKEY, description = "The IAM Secret Access Key to use for authentication.", required = "If present, performs explicit IAM authentication")
@RespectsArgumentUri
    ...
    * https://s3.amazonaws.com/redshift-downloads/drivers/jdbc/1.2.43.1067/Amazon+Redshift+JDBC+Driver+Install+Guide.pdf
    ...
    private String makeJdbcUrlRedshiftIAM(ConnectorArguments arguments) throws MetadataDumperUsageException, UnsupportedEncodingException {
        ...
        if (arguments.getIAMProfile() != null)
            url += new JdbcPropBuilder("?=&")
                    .prop("Profile", arguments.getIAMProfile())
                    .toJdbcPart();
        ...
        //throw new MetadataDumperUsageException("Either --iam-profile or --iam-accesskeyid/--iam-secretaccesskey should be given");
        return url;
    }
        ...
        if (url == null) {
            ...
            if (!isDriverRedshift) {
                if (!isAuthenticationPassword)
                    throw new IllegalArgumentException("The use of IAM authentication also requires the use of a Redshift-specific JDBC driver. Please use --" + ConnectorArguments.OPT_DRIVER + " to specify the path to the Redshift JDBC JAR, or use password authentication.");
