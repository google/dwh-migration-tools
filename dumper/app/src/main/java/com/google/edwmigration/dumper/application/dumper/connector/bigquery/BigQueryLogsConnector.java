...
public class BigQueryLogsConnector extends AbstractBigQueryConnector implements LogsConnector, BigQueryLogsDumpFormat {
    ...
    public static class QueryLogsTask extends AbstractBigQueryTask implements BigQueryLogsDumpFormat.QueryLogsTask {
        ...
        private final BigQuery.JobListOption jloPageSize = BigQuery.JobListOption.pageSize(10000);   // Is there a server side limit of 1000?
        private final BigQuery.JobListOption jloState = BigQuery.JobListOption.stateFilter(JobStatus.State.DONE);
        ...
        protected void run(@Nonnull Writer writer, @Nonnull BigQuery bigQuery) throws Exception {
            // BigQuery.JobListOption allUsers = BigQuery.JobListOption.allUsers(); //XXX: using allUsers causes NPE while getting values.

            List<? extends String> projectIds = arguments.getDatabases();
            if (projectIds.isEmpty())
                projectIds = ImmutableList.of(bigQuery.getOptions().getProjectId());

            ZonedIntervalIterable intervals = ZonedIntervalIterable.forConnectorArguments(arguments);

            //Currently uses Jobs API to get list of query jobs (https://cloud.google.com/bigquery/docs/reference/rest/v2/jobs/list)
            ...
            try (
                    ConcurrentProgressMonitor monitor = new ConcurrentRecordProgressMonitor("Writing to " + getTargetPath());
                    ExecutorManager manager = new ExecutorManager(executor)) {
                for (String projectId : projectIds) {
                    for (ZonedInterval interval : intervals) {
                        LOG.info("Retrieving query logs in range start={} to end={}", interval.getStart(), interval.getEndExclusive());
                        BigQuery.JobListOption minCreationTimeMillis = BigQuery.JobListOption.minCreationTime(interval.getStart().toEpochSecond() * 1000);
                        BigQuery.JobListOption maxCreationTimeMillis = BigQuery.JobListOption.maxCreationTime(interval.getEndExclusive().toEpochSecond() * 1000);
                        Page<Job> jobPage = runWithBackOff(() -> BigQueryAccessor.listJobs(bigQuery, projectId, minCreationTimeMillis, maxCreationTimeMillis, jloAllUsers, jloPageSize, jloState));
        ...
        private void writeJobsDetails(@Nonnull ExecutorManager manager, @Nonnull BigQuery bigQuery, @Nonnull Writer writer, @Nonnull Iterable<Job> jobs, @Nonnull ConcurrentProgressMonitor monitor) throws Exception {
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    LOG_LIMITED.warn("Failed to load job (attempting to continue): " + monitor.getCount() + ": " + e, e);
                    continue;
                }
                ...
                if (jobId == null || jobId.getJob() == null) {
                    if (DEBUG)
                        LOG_LIMITED.debug("No JobId: Assuming dry run. Skipping.");
                ...
                if (statistics == null) {
                    if (DEBUG)
                        LOG_LIMITED.debug("No JobStatistics in {}. Skipping. You need bigquery.jobs.listAll permission or bigquery.admin role to list all jobs.", jobId);
                    continue;
                }
                ...
                if (jobChildCount != null && jobChildCount > 0) {
                    if (DEBUG)
                        LOG_LIMITED.debug("Listing {} children of {}.{}", jobChildCount, jobId.getProject(), jobId);
                    BigQuery.JobListOption jloParent = BigQuery.JobListOption.parentJobId(jobId.getJob());
                    manager.execute(() -> {
                        Page<Job> jobChildPage = runWithBackOff(() -> BigQueryAccessor.listJobs(bigQuery, jobId.getProject(), jloParent, jloPageSize));
                ...
                if (configuration == null) {
                    if (DEBUG)
                        LOG_LIMITED.debug("No JobConfiguration in job: " + job);
                    continue;
                }
                ...
                if (type != null) {
                    switch (type) {
                        case QUERY:
                            ...
                            if (BooleanUtils.isTrue(queryJobConfiguration.dryRun()))
                                continue;
                            addQueryJob(out, job);
                            break;
                        case LOAD:
        ...
        private static void addQueryJob(@Nonnull QueryHistoryJson.Job out, @Nonnull Job job) {
            QueryHistoryJson.QueryJobConfiguration outConfiguration = new QueryHistoryJson.QueryJobConfiguration();
            ...
            DESTINATION:
            {
                ...
                outConfiguration.writeDisposition = getEnumName(queryJobConfiguration.getWriteDisposition());
        ...
        private static void addLoadJob(@Nonnull QueryHistoryJson.Job out, @Nonnull Job job) {
            QueryHistoryJson.LoadJobConfiguration outConfiguration = new QueryHistoryJson.LoadJobConfiguration();
    ...
    @Override
    // TODO: Use ZonedIntervalIterable.forConnectorArguments() to generate N tasks here, each for a window, rather than just using one task with arguments.
