...
public class BigQueryLogsConnector extends AbstractBigQueryConnector implements LogsConnector,
    BigQueryLogsDumpFormat {
  ...
  public static class QueryLogsTask extends AbstractBigQueryTask implements
      BigQueryLogsDumpFormat.QueryLogsTask {

    private final ConnectorArguments arguments;
    private final BigQuery.JobListOption jloPageSize = BigQuery.JobListOption.pageSize(
        10000);   // Is there a server side limit of 1000?
    private final BigQuery.JobListOption jloState = BigQuery.JobListOption.stateFilter(
    ...
    protected void run(@Nonnull Writer writer, @Nonnull BigQuery bigQuery) throws Exception {
      ...
      try (
          ConcurrentProgressMonitor monitor = new ConcurrentRecordProgressMonitor(
              "Writing to " + getTargetPath());
          ExecutorManager manager = new ExecutorManager(executor)) {
        for (String projectId : projectIds) {
          for (ZonedInterval interval : intervals) {
            LOG.info("Retrieving query logs in range start={} to end={}", interval.getStart(),
                interval.getEndExclusive());
            BigQuery.JobListOption minCreationTimeMillis = BigQuery.JobListOption.minCreationTime(
                interval.getStart().toEpochSecond() * 1000);
            BigQuery.JobListOption maxCreationTimeMillis = BigQuery.JobListOption.maxCreationTime(
                interval.getEndExclusive().toEpochSecond() * 1000);
            Page<Job> jobPage = runWithBackOff(
                () -> BigQueryAccessor.listJobs(bigQuery, projectId, minCreationTimeMillis,
                    maxCreationTimeMillis, jloAllUsers, jloPageSize, jloState));
    ...
    private void writeJobsDetails(@Nonnull ExecutorManager manager, @Nonnull BigQuery bigQuery,
        @Nonnull Writer writer, @Nonnull Iterable<Job> jobs,
        @Nonnull ConcurrentProgressMonitor monitor) throws Exception {
      ...
      while (it.hasNext()) {
        ...
        if (jobChildCount != null && jobChildCount > 0) {
          if (DEBUG) {
            LOG_LIMITED.debug("Listing {} children of {}.{}", jobChildCount, jobId.getProject(),
                jobId);
          }
          BigQuery.JobListOption jloParent = BigQuery.JobListOption.parentJobId(jobId.getJob());
          manager.execute(() -> {
            Page<Job> jobChildPage = runWithBackOff(
                () -> BigQueryAccessor.listJobs(bigQuery, jobId.getProject(), jloParent,
                    jloPageSize));
    ...
    private static void addQueryJob(@Nonnull QueryHistoryJson.Job out, @Nonnull Job job) {
      ...
      DESTINATION:
      {
        outConfiguration.createDisposition = getEnumName(
            queryJobConfiguration.getCreateDisposition());
        outConfiguration.writeDisposition = getEnumName(
