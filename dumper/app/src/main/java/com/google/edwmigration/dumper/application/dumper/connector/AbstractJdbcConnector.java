...
public abstract class AbstractJdbcConnector extends AbstractConnector {
     ...
     * @param driverPath A comma-separated list of paths to JAR files for inclusion in the new ClassLoader.
     * @return The JDBC ClassLoader to use to load the Driver.
     * @throws PrivilegedActionException
     * @throws MalformedURLException
     */
    @Nonnull
    private static ClassLoader newDriverClassLoader(@Nonnull ClassLoader parentClassLoader, @CheckForNull List<? extends String> driverPaths) throws PrivilegedActionException, MalformedURLException {
        ...
        for (String driverPath : driverPaths) {
            ...
            if (!driverFile.isFile())
                throw new IllegalArgumentException("Not a driver JAR-file: " + driverFile.getAbsolutePath());
    ...
    @Nonnull
    protected Driver newDriver(@CheckForNull List<? extends String> driverPaths, @Nonnull String... driverClassNames) throws SQLException {
                ...
                for (String driverClassName : driverClassNames) {
                    try {
                        ...
                        if (e.getCause() instanceof ClassNotFoundException)
                            LOG.warn("Cannot load " + driverClassName + " from " + driverPaths + ": " + e.getCause());
                        else
                            throw e;
                    }
                }
                throw new SQLException("Failed to load any of " + Arrays.toString(driverClassNames) + " from " + driverPaths);
            }

            LOG.info("Using JDBC Driver " + driverClass);
            return driverClass.asSubclass(Driver.class).getConstructor().newInstance();
        } catch (ReflectiveOperationException | PrivilegedActionException | MalformedURLException | RuntimeException e) {
            throw new SQLException("Failed to load or instantiate " + driverClass + " from " + driverPaths + ": " + e, e);
        }
    }
    ...
    @Nonnull
    protected SimpleDriverDataSource newSimpleDataSource(@Nonnull Driver driver, @Nonnull String url, @Nonnull ConnectorArguments arguments) throws SQLException, MetadataDumperUsageException {
        if (!driver.acceptsURL(url))
            throw new MetadataDumperUsageException("JDBC driver " + driver + " does not accept URL " + url);
