...
@RespectsInput(order = ConnectorArguments.OPT_PORT_ORDER, arg = ConnectorArguments.OPT_PORT, description = "The port of the server.", required = ConnectorArguments.OPT_REQUIRED_IF_NOT_URL, defaultValue =
    "" + NetezzaMetadataConnector.OPT_PORT_DEFAULT)
@RespectsArgumentUser
@RespectsArgumentPassword
@RespectsArgumentUri
@RespectsArgumentDatabasePredicate
public class NetezzaMetadataConnector extends AbstractJdbcConnector implements MetadataConnector,
    NetezzaMetadataDumpFormat {
  ...
  private static class NetezzaDatabaseListTask extends AbstractJdbcTask<List<String>> {
    ...
    protected ResultSetExtractor<List<String>> newResultSetExtractor(@Nonnull ByteSink sink,
    ...
    @Override
    protected List<String> doInConnection(TaskRunContext context, JdbcHandle jdbcHandle,
        ByteSink sink, Connection connection) throws SQLException {
    ...
    public List<String> run(TaskRunContext context) throws Exception {
      ...
      for (String databaseName : MoreObjects.firstNonNull(databaseNames,
  ...
  public void addTasksTo(List<? super Task<?>> out, ConnectorArguments arguments) {
    ...
    out.add(new JdbcSelectTask("nz.version.csv",
        "SELECT 'netezza' AS dialect, "
            // CSVPrinter cannot handle the type:
            + "cast(version() as varchar(64)) as version, "
            // " + db + ".admin.admin._v_system_info.SYSTEM_SOFTWARE_VERSION
    ...
    // objid, Operator, Owner, CreateDate, Description, oprname, oprleft, oprright, oprresult, oprcode, oprkind [order by Operator]
    ...
    for (String db : dbs) {
      ...
      // Also, case-sensitivity concerns (NZ is not always uppercase; `select identifier_case;`) suggests that this filter is hard to construct reliably.
      ...
      String schemaPrefix = db
          + "..";    // We don't know what the default schema is, but it should contain the views we require.
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "netezza.columns.csv"),
          "SELECT DATABASE, TABLE_SCHEM, TABLE_NAME, COLUMN_NAME, TYPE_NAME FROM " + schemaPrefix
              + "_v_sys_columns ORDER BY TABLE_NAME, OBJID"));

      // objid, Function, Owner, CreateDate, Description, Result, Arguments [order by Function]
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_function.csv"),
          "SELECT * FROM " + schemaPrefix + "_v_function"));
      // objid, IndexName, TableName, Owner, CreateDate [order by TableName, IndexName]
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_index.csv"),
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_procedure.csv"),
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_relation_column.csv"),
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_relation_column_def.csv"),
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_relation_keydata.csv"),
      ...
      out.add(new JdbcSelectTask(withPrefix(filePrefix, Views.ZIP_ENTRY_SUFFIX),
          "SELECT * FROM " + schemaPrefix + "_v_view"));

      // Not documented.
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_sys_columns.csv"),
          "SELECT * FROM " + schemaPrefix + "_v_sys_columns"));
      // Not documented.
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_sys_database.csv"),
          "SELECT * FROM " + schemaPrefix + "_v_sys_database"));
      // objid, SysTableName, and Owner [order by SysTableName]
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_sys_table.csv"),
          "SELECT * FROM " + schemaPrefix + "_v_sys_table"));
      // objid, SysViewName, and Owner [order by SysViewName]
      out.add(new JdbcSelectTask(withPrefix(filePrefix, "nz.v_sys_view.csv"),
