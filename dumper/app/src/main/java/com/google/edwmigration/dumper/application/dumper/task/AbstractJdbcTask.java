...
public abstract class AbstractJdbcTask<T> extends AbstractTask<T> {
  ...
  @Nonnull
  protected ResultSetExtractor<Void> newCsvResultSetExtractor(@Nonnull ByteSink sink,
      @CheckForSigned long count) {
    return rs -> {
  ...
  public static void setParameterValues(@Nonnull PreparedStatement statement, Object... arguments)
    ...
    for (int i = 0; i < arguments.length; i++) {
      StatementCreatorUtils.setParameterValue(statement, i + 1, SqlTypeValue.TYPE_UNKNOWN,
          arguments[i]);
    }
  }
  ...
  private static void debug(@Nonnull Statement statement) throws SQLException {
    LOG.debug("Concurrency = " + statement.getResultSetConcurrency() + " (want "
        + ResultSet.CONCUR_READ_ONLY + ")");
    LOG.debug("Holdability = " + statement.getResultSetHoldability() + " (want "
        + ResultSet.CLOSE_CURSORS_AT_COMMIT + ")");
    LOG.debug("FetchSize = " + statement.getFetchSize());
    LOG.debug(
        "ResultSetType = " + statement.getResultSetType() + " (want " + ResultSet.TYPE_FORWARD_ONLY
            + ")");
  ...
  @CheckForNull
  protected static <T> T doSelect(@Nonnull Connection connection,
      @Nonnull ResultSetExtractor<T> resultSetExtractor, @Nonnull String sql,
      @Nonnull Object... arguments) throws SQLException {
    ...
    try {
      ...
      while (warning != null) {
        LOG.warn("SQL warning: [" + warning.getSQLState() + "/" + warning.getErrorCode() + "] "
            + warning.getMessage());
  ...
  @CheckForNull
  protected abstract T doInConnection(@Nonnull TaskRunContext context,
      @Nonnull JdbcHandle jdbcHandle, @Nonnull ByteSink sink, @Nonnull Connection connection)
      throws SQLException;
