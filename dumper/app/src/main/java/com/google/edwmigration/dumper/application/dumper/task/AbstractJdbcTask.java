...
public abstract class AbstractJdbcTask<T> extends AbstractTask<T> {
        ...
        if (headerClass != null) {
            ...
            if (headerClass.getEnumConstants().length != rs.getMetaData().getColumnCount())
                ...
                throw new SQLException(
                        new MetadataDumperUsageException("Fatal Error. ResultSet does not have the expected column count: " + headerClass.getEnumConstants().length,
                                Arrays.asList("If a custom query has been specified please confirm the selected columns match the following: ",
    ...
    public static void setParameterValues(@Nonnull PreparedStatement statement, Object... arguments) throws SQLException {
        for (int i = 0; i < arguments.length; i++)
            StatementCreatorUtils.setParameterValue(statement, i + 1, SqlTypeValue.TYPE_UNKNOWN, arguments[i]);
    }

    @SuppressWarnings("UnusedMethod")
    private static void debug(@Nonnull Statement statement) throws SQLException {
        LOG.debug("Concurrency = " + statement.getResultSetConcurrency() + " (want " + ResultSet.CONCUR_READ_ONLY + ")");
        LOG.debug("Holdability = " + statement.getResultSetHoldability() + " (want " + ResultSet.CLOSE_CURSORS_AT_COMMIT + ")");
        LOG.debug("FetchSize = " + statement.getFetchSize());
        LOG.debug("ResultSetType = " + statement.getResultSetType() + " (want " + ResultSet.TYPE_FORWARD_ONLY + ")");
    ...
    @CheckForNull
    protected static <T> T doSelect(@Nonnull Connection connection, @Nonnull ResultSetExtractor<T> resultSetExtractor, @Nonnull String sql, @Nonnull Object... arguments) throws SQLException {
        ...
        try {
            ...
            PREPARE:
            {
                ...
                // connection.setHoldability(ResultSet.CLOSE_CURSORS_AT_COMMIT);    // Shouldn't be required.
                statement = connection.prepareStatement(sql, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
                setParameterValues(statement, arguments);
                // statement.setFetchDirection(ResultSet.FETCH_FORWARD);   // PostgreSQL and (allegedly) Teradata prefer this. However, it is the default, and sqlite throws.
            ...
            while (warning != null) {
                LOG.warn("SQL warning: [" + warning.getSQLState() + "/" + warning.getErrorCode() + "] " + warning.getMessage());
