...
public class OracleMetadataConnector extends AbstractOracleConnector implements MetadataConnector,
    OracleMetadataDumpFormat {
  ...
  private static class SelectXmlTask extends AbstractJdbcTask<Void> implements GroupTask {
     ...
     */
    public SelectXmlTask(String targetPath, String rowSql, String xmlSql, int ownerIndex,
        int nameIndex) {
      super(targetPath);
    ...
    @Nonnull
    private ResultSetExtractor<Void> newResultSetExtractor(@Nonnull ByteSink sink,
        @Nonnull JdbcHandle handle) {
    ...
    @Override
    protected Void doInConnection(@Nonnull TaskRunContext context, @Nonnull JdbcHandle jdbcHandle,
  ...
  private static class MessageTask extends AbstractTask<Void> {
    ...
    protected Void doRun(TaskRunContext context, @Nonnull ByteSink sink, @Nonnull Handle handle)
      ...
      for (GroupTask task : tasks) {
        LOG.error("(" + c + "): " + task.getName() + " : " + ExceptionUtils.getRootCauseMessage(
            task.getException()));
        c += 1;
      }
      return null;
    ...
    @Override
    public String toString() {
      return "[ Error if all fail: " + String.join(", ",
          Lists.transform(Arrays.asList(tasks), GroupTask::getName)) + " ]";
    }
  }

  private static void addAtLeastOneOf(@Nonnull List<? super Task<?>> out,
      @Nonnull GroupTask... tasks) {
    for (GroupTask task : tasks) {
  ...
  @Nonnull
  private static SelectTask newSelectStarTask(@Nonnull String file, @Nonnull String table,
      @Nonnull String where) {
    return new SelectTask(file, "SELECT * FROM " + table + where);
  }

  private static void buildSelectStarTask(List<? super Task<?>> out, String all_file,
      String all_table, String dba_file, String dba_table, @Nonnull String whereCond) {
  ...
  public void addTasksTo(List<? super Task<?>> out, @Nonnull ConnectorArguments arguments)
    ...
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlTables.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_TABLES" + whereCondOwner,
            xmlQuery),
        new SelectXmlTask(XmlTables.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_TABLES" + whereCondOwner,
            xmlQuery));

    sqlQuery = "SELECT OWNER, VIEW_NAME FROM ";
    xmlQuery = "SELECT DBMS_METADATA.GET_XML('VIEW', ?, ?) FROM DUAL";
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlViews.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_VIEWS" + whereCondOwner,
            xmlQuery),
        new SelectXmlTask(XmlViews.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_VIEWS" + whereCondOwner,
            xmlQuery));

    sqlQuery = "SELECT OWNER, INDEX_NAME FROM ";
    xmlQuery = "SELECT DBMS_METADATA.GET_XML('INDEX', INDEX_NAME) FROM DUAL";
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlIndexes.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_INDEXES" + whereCondOwner,
            xmlQuery),
        new SelectXmlTask(XmlIndexes.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_INDEXES" + whereCondOwner,
            xmlQuery));

    sqlQuery = "SELECT SEQUENCE_OWNER, SEQUENCE_NAME FROM ";
    xmlQuery = "SELECT DBMS_METADATA.GET_XML('SEQUENCE', SEQUENCE_NAME) FROM DUAL";
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlSequences.ZIP_ENTRY_NAME_DBA,
            sqlQuery + "DBA_SEQUENCES" + whereCondSequenceOwner, xmlQuery),
        new SelectXmlTask(XmlSequences.ZIP_ENTRY_NAME_ALL,
            sqlQuery + "ALL_SEQUENCES" + whereCondSequenceOwner, xmlQuery));

    sqlQuery = "SELECT OWNER, TYPE_NAME FROM ";
    xmlQuery = "SELECT DBMS_METADATA.GET_XML('TYPE', TYPE_NAME) FROM DUAL";
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlTypes.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_TYPES" + whereCondOwner,
            xmlQuery),
        new SelectXmlTask(XmlTypes.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_TYPES" + whereCondOwner,
            xmlQuery));

    sqlQuery = "SELECT OWNER, SYNONYM_NAME FROM ";
    xmlQuery = "SELECT DBMS_METADATA.GET_XML('SYNONYM', SYNONYM_NAME) FROM DUAL";
    addAtLeastOneOf(out,
        new SelectXmlTask(XmlSynonyms.ZIP_ENTRY_NAME_DBA,
            sqlQuery + "DBA_SYNONYMS" + whereCondOwner, xmlQuery),
        new SelectXmlTask(XmlSynonyms.ZIP_ENTRY_NAME_ALL,
            sqlQuery + "ALL_SYNONYMS" + whereCondOwner, xmlQuery));
