...
public class OracleMetadataConnector extends AbstractOracleConnector implements MetadataConnector, OracleMetadataDumpFormat {
    ...
    private static class SelectXmlTask extends AbstractJdbcTask<Void> implements GroupTask {
         ...
         */
        public SelectXmlTask(String targetPath, String rowSql, String xmlSql, int ownerIndex, int nameIndex) {
            super(targetPath);
        ...
        @Nonnull
        private ResultSetExtractor<Void> newResultSetExtractor(@Nonnull ByteSink sink, @Nonnull JdbcHandle handle) {
    ...
    private static class MessageTask extends AbstractTask<Void> {
        ...
        protected Void doRun(TaskRunContext context, @Nonnull ByteSink sink, @Nonnull Handle handle) throws Exception {
            ...
            for (GroupTask task : tasks) {
                LOG.error("(" + c + "): " + task.getName() + " : " + ExceptionUtils.getRootCauseMessage(task.getException()));
                c += 1;
            }
            return null;
        }
        ...
        @Override
        public String toString() {
            return "[ Error if all fail: " + String.join(", ", Lists.transform(Arrays.asList(tasks), GroupTask::getName)) + " ]";
        }
    }

    private static void addAtLeastOneOf(@Nonnull List<? super Task<?>> out, @Nonnull GroupTask... tasks) {
        for (GroupTask task : tasks)
            out.add(Preconditions.checkNotNull(task));
    ...
    @Nonnull
    private static SelectTask newSelectStarTask(@Nonnull String file, @Nonnull String table, @Nonnull String where) {
        return new SelectTask(file, "SELECT * FROM " + table + where);
    }

    private static void buildSelectStarTask(List<? super Task<?>> out, String all_file, String all_table, String dba_file, String dba_table, @Nonnull String whereCond) {
    ...
    @Override
    public void addTasksTo(List<? super Task<?>> out, @Nonnull ConnectorArguments arguments) throws Exception {
        ...
        String whereCondOwner = ownerInList == null
                ? ""
                : " WHERE OWNER IN " + ownerInList;
        String whereCondTableOwner = ownerInList == null
                ? ""
                : " WHERE TABLE_OWNER IN " + ownerInList;
        String whereCondSequenceOwner = ownerInList == null
                ? ""
                : " WHERE SEQUENCE_OWNER IN " + ownerInList;

        buildSelectStarTask(out,
                Arguments.ZIP_ENTRY_NAME_DBA, "DBA_Arguments",
                Arguments.ZIP_ENTRY_NAME_ALL, "All_Arguments",
                whereCondOwner);
        buildSelectStarTask(out,
                Catalog.ZIP_ENTRY_NAME_DBA, "DBA_Catalog",
                Catalog.ZIP_ENTRY_NAME_ALL, "All_Catalog",
                whereCondOwner);
        buildSelectStarTask(out,
                Constraints.ZIP_ENTRY_NAME_DBA, "DBA_Constraints",
                Constraints.ZIP_ENTRY_NAME_ALL, "All_Constraints",
                whereCondOwner);
        buildSelectStarTask(out,
                Indexes.ZIP_ENTRY_NAME_DBA, "DBA_Indexes",
                Indexes.ZIP_ENTRY_NAME_ALL, "All_Indexes",
                whereCondOwner);
        buildSelectStarTask(out,
                MViews.ZIP_ENTRY_NAME_DBA, "DBA_MViews",
                MViews.ZIP_ENTRY_NAME_ALL, "All_MViews",
                whereCondOwner);
        buildSelectStarTask(out,
                Operators.ZIP_ENTRY_NAME_DBA, "DBA_Operators",
                Operators.ZIP_ENTRY_NAME_ALL, "All_Operators",
                whereCondOwner);
        buildSelectStarTask(out,
                Part_key_columns.ZIP_ENTRY_NAME_DBA, "DBA_Part_key_columns",
                Part_key_columns.ZIP_ENTRY_NAME_ALL, "All_Part_key_columns",
                whereCondOwner);
        buildSelectStarTask(out,
                Plsql_Types.ZIP_ENTRY_NAME_DBA, "DBA_Plsql_Types",
                Plsql_Types.ZIP_ENTRY_NAME_ALL, "All_Plsql_Types",
                whereCondOwner);
        buildSelectStarTask(out,
                Procedures.ZIP_ENTRY_NAME_DBA, "DBA_Procedures",
                Procedures.ZIP_ENTRY_NAME_ALL, "All_Procedures",
                whereCondOwner);
        buildSelectStarTask(out,
                Tab_Columns.ZIP_ENTRY_NAME_DBA, "DBA_Tab_Columns",
                Tab_Columns.ZIP_ENTRY_NAME_ALL, "All_Tab_Columns",
                whereCondOwner);
        buildSelectStarTask(out,
                Tab_Partitions.ZIP_ENTRY_NAME_DBA, "DBA_Tab_Partitions",
                Tab_Partitions.ZIP_ENTRY_NAME_ALL, "All_Tab_Partitions",
                whereCondTableOwner);
        buildSelectStarTask(out,
                Tables.ZIP_ENTRY_NAME_DBA, "DBA_Tables",
                Tables.ZIP_ENTRY_NAME_ALL, "All_Tables",
                whereCondOwner);
        buildSelectStarTask(out,
                Types.ZIP_ENTRY_NAME_DBA, "DBA_Types",
                Types.ZIP_ENTRY_NAME_ALL, "All_Types",
                whereCondOwner);
        buildSelectStarTask(out,
                Views.ZIP_ENTRY_NAME_DBA, "DBA_Views",
                Views.ZIP_ENTRY_NAME_ALL, "All_Views",
                whereCondOwner);
        ...
        String whereCondFunctionOwner = ownerInList == null
                ? ""
                : " AND OWNER IN " + ownerInList;
        out.add(new SelectXmlTask(XmlFunctions.ZIP_ENTRY_NAME,
                "SELECT OWNER, OBJECT_NAME FROM ALL_OBJECTS WHERE OBJECT_NAME = 'FUNCTION'" + whereCondFunctionOwner,
        ...
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlTables.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_TABLES" + whereCondOwner, xmlQuery),
                new SelectXmlTask(XmlTables.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_TABLES" + whereCondOwner, xmlQuery));

        sqlQuery = "SELECT OWNER, VIEW_NAME FROM ";
        xmlQuery = "SELECT DBMS_METADATA.GET_XML('VIEW', ?, ?) FROM DUAL";
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlViews.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_VIEWS" + whereCondOwner, xmlQuery),
                new SelectXmlTask(XmlViews.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_VIEWS" + whereCondOwner, xmlQuery));

        sqlQuery = "SELECT OWNER, INDEX_NAME FROM ";
        xmlQuery = "SELECT DBMS_METADATA.GET_XML('INDEX', INDEX_NAME) FROM DUAL";
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlIndexes.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_INDEXES" + whereCondOwner, xmlQuery),
                new SelectXmlTask(XmlIndexes.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_INDEXES" + whereCondOwner, xmlQuery));

        sqlQuery = "SELECT SEQUENCE_OWNER, SEQUENCE_NAME FROM ";
        xmlQuery = "SELECT DBMS_METADATA.GET_XML('SEQUENCE', SEQUENCE_NAME) FROM DUAL";
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlSequences.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_SEQUENCES" + whereCondSequenceOwner, xmlQuery),
                new SelectXmlTask(XmlSequences.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_SEQUENCES" + whereCondSequenceOwner, xmlQuery));

        sqlQuery = "SELECT OWNER, TYPE_NAME FROM ";
        xmlQuery = "SELECT DBMS_METADATA.GET_XML('TYPE', TYPE_NAME) FROM DUAL";
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlTypes.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_TYPES" + whereCondOwner, xmlQuery),
                new SelectXmlTask(XmlTypes.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_TYPES" + whereCondOwner, xmlQuery));

        sqlQuery = "SELECT OWNER, SYNONYM_NAME FROM ";
        xmlQuery = "SELECT DBMS_METADATA.GET_XML('SYNONYM', SYNONYM_NAME) FROM DUAL";
        addAtLeastOneOf(out,
                new SelectXmlTask(XmlSynonyms.ZIP_ENTRY_NAME_DBA, sqlQuery + "DBA_SYNONYMS" + whereCondOwner, xmlQuery),
                new SelectXmlTask(XmlSynonyms.ZIP_ENTRY_NAME_ALL, sqlQuery + "ALL_SYNONYMS" + whereCondOwner, xmlQuery));
