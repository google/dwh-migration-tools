...
public class RedshiftLogsConnector extends AbstractRedshiftConnector implements LogsConnector,
    RedshiftLogsDumpFormat {
  ...
  public void addTasksTo(List<? super Task<?>> out, ConnectorArguments arguments)
    ...
    makeTasks(arguments, RedshiftLogsDumpFormat.DdlHistory.ZIP_ENTRY_PREFIX, queryTemplateDDL,
        "L.starttime", parallelTask);
    ...
    makeTasks(arguments, RedshiftLogsDumpFormat.QueryHistory.ZIP_ENTRY_PREFIX, queryTemplateQuery,
        "Q.starttime", parallelTask);
    ...
    if (false) {
      ...
      makeTasks(arguments, RedshiftLogsDumpFormat.ZIP_ENTRY_PREFIX_METRICS, queryTemplateMetrics,
          "Q.starttime", parallelTask);

      // We want to know how many bytes were scanned per query, so we must consult STL_SCAN;
      // SVL_QUERY_METRICS already gives us "scan_row_count", but doesn't tell us anything about bytes scanned.
      ...
      makeTasks(arguments, RedshiftLogsDumpFormat.ZIP_ENTRY_PREFIX_SCANS, queryTemplateScan,
          "Q.starttime", parallelTask);
    }
  }
    ...
    for (ZonedInterval interval : intervals) {
      String query = queryTemplate.replace("##",
          newWhereClause(whereClauses,
              String.format("%s >= CAST('%s' AS TIMESTAMP)", startField,
