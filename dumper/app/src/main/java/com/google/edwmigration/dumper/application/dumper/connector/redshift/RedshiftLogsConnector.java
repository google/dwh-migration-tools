...
public class RedshiftLogsConnector extends AbstractRedshiftConnector implements LogsConnector, RedshiftLogsDumpFormat {
    ...
    public void addTasksTo(List<? super Task<?>> out, ConnectorArguments arguments) throws MetadataDumperUsageException {
        ...
        if (false) {
            ...
            makeTasks(arguments, RedshiftLogsDumpFormat.ZIP_ENTRY_PREFIX_METRICS, queryTemplateMetrics, "Q.starttime", parallelTask);
            ...
            makeTasks(arguments, RedshiftLogsDumpFormat.ZIP_ENTRY_PREFIX_SCANS, queryTemplateScan, "Q.starttime", parallelTask);
        }
    }
        ...
        for (ZonedInterval interval : intervals) {
            String query = queryTemplate.replace("##",
                    newWhereClause(whereClauses,
                            ...
                            String.format("%s < CAST('%s' AS TIMESTAMP)", startField, SQL_FORMAT.format(interval.getEndExclusive()))));
            String file = filePrefix + DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(interval.getStartUTC()) + RedshiftLogsDumpFormat.ZIP_ENTRY_SUFFIX;
