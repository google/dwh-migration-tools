...
public class SnowflakeMetadataConnector extends AbstractSnowflakeConnector implements MetadataConnector, SnowflakeMetadataDumpFormat {
    ...
    protected void addSqlTasks(@Nonnull List<? super Task<?>> out, @Nonnull Class<? extends Enum<?>> header, @Nonnull String format, @Nonnull TaskVariant is_task, @Nonnull TaskVariant au_task) {
        ...
        Task<?> t1 = new JdbcSelectTask(au_task.zipEntryName, String.format(format, au_task.schemaName, au_task.whereClause))
                .withHeaderClass(header)
                .onlyIfFailed(t0);
        out.add(t0);
        out.add(t1);
    ...
    public void addTasksTo(@Nonnull List<? super Task<?>> out, @Nonnull ConnectorArguments arguments) {
        ...
        // ACCOUNT_USAGE is much faster than INFORMATION_SCHEMA and does not have the size limitations, but requires extra privileges to be granted.
        ...
        addSqlTasks(out, SnowflakeMetadataDumpFormat.DatabasesFormat.Header.class,
                getOverrideableQuery(arguments, "SELECT database_name, database_owner FROM %1$s.DATABASES%2$s",
                        SnowflakeMetadataConnectorProperties.DATABASES_OVERRIDE_QUERY, SnowflakeMetadataConnectorProperties.DATABASES_OVERRIDE_WHERE),
                new TaskVariant(SnowflakeMetadataDumpFormat.DatabasesFormat.IS_ZIP_ENTRY_NAME, IS),
                new TaskVariant(SnowflakeMetadataDumpFormat.DatabasesFormat.AU_ZIP_ENTRY_NAME, AU, AU_WHERE));

        addSqlTasks(out, SnowflakeMetadataDumpFormat.SchemataFormat.Header.class,
                getOverrideableQuery(arguments, "SELECT catalog_name, schema_name FROM %1$s.SCHEMATA%2$s",
                        SnowflakeMetadataConnectorProperties.SCHEMATA_OVERRIDE_QUERY, SnowflakeMetadataConnectorProperties.SCHEMATA_OVERRIDE_WHERE),
        ...
        addSqlTasks(out, SnowflakeMetadataDumpFormat.ColumnsFormat.Header.class,
                getOverrideableQuery(arguments, "SELECT table_catalog, table_schema, table_name, ordinal_position, column_name, data_type FROM %1$s.COLUMNS%2$s",
                        SnowflakeMetadataConnectorProperties.COLUMNS_OVERRIDE_QUERY, SnowflakeMetadataConnectorProperties.COLUMNS_OVERRIDE_WHERE),
                new TaskVariant(SnowflakeMetadataDumpFormat.ColumnsFormat.IS_ZIP_ENTRY_NAME, IS),
                new TaskVariant(SnowflakeMetadataDumpFormat.ColumnsFormat.AU_ZIP_ENTRY_NAME, AU, AU_WHERE));    // Very fast.

        addSqlTasks(out, SnowflakeMetadataDumpFormat.ViewsFormat.Header.class,
                getOverrideableQuery(arguments, "SELECT table_catalog, table_schema, table_name, view_definition FROM %1$s.VIEWS%2$s",
                        SnowflakeMetadataConnectorProperties.VIEWS_OVERRIDE_QUERY, SnowflakeMetadataConnectorProperties.VIEWS_OVERRIDE_WHERE),
        ...
        addSqlTasks(out, SnowflakeMetadataDumpFormat.FunctionsFormat.Header.class,
                getOverrideableQuery(arguments, "SELECT function_schema, function_name, data_type, argument_signature FROM %1$s.FUNCTIONS%2$s",
                        SnowflakeMetadataConnectorProperties.FUNCTIONS_OVERRIDE_QUERY, SnowflakeMetadataConnectorProperties.FUNCTIONS_OVERRIDE_WHERE),
                new TaskVariant(SnowflakeMetadataDumpFormat.FunctionsFormat.IS_ZIP_ENTRY_NAME, IS),
                new TaskVariant(SnowflakeMetadataDumpFormat.FunctionsFormat.AU_ZIP_ENTRY_NAME, AU, AU_WHERE));
    }
