...
public abstract class AbstractHiveConnector extends AbstractConnector {
  ...
  public static class ThriftClientPool implements AutoCloseable {
    ...
    public ThriftClientPool(@Nonnull String name,
        @Nonnull HiveMetastoreThriftClient.Builder thriftClientBuilder, int threadPoolSize) {
      this.name = Preconditions.checkNotNull(name, "name was null.");
      this.executorService = ExecutorManager.newExecutorServiceWithBackpressure(name,
          threadPoolSize);
      this.executorManager = new ExecutorManager(executorService);
      this.threadLocalThriftClient = ThreadLocal.withInitial(() -> {
        String threadName = Thread.currentThread().getName();
        LOG.debug("Creating new thread-local Thrift client '{}' owned by pooled client '{}'.",
            threadName, name);
        try {
          HiveMetastoreThriftClient client = new HiveMetastoreThriftClient.Builder(
              thriftClientBuilder).withName(threadName).build();
          synchronized (lock) {
          ...
          throw new RuntimeException(
              "Unable to build Thrift client '" + threadName + "' owned by pooled client '" + name
                  + "' due to transport exception.", e);
        }
      });
    }
    ...
    public void close() throws Exception {
      ...
      synchronized (lock) {
        for (HiveMetastoreThriftClient client : builtClients) {
          try {
            LOG.debug("Closing thread-local Thrift client '{}' owned by pooled client '{}'.",
                client.getName(), name);
            client.close();
          } catch (Exception ioe) {
            LOG.warn(
                "Unable to close Thrift client '" + client.getName() + "' owned by pooled client '"
                    + name + "'. " + client.getName(), ioe);
          }
        }
      }
  ...
  protected static class ThriftClientHandle extends AbstractHandle {
    ...
    public ThriftClientHandle(@Nonnull HiveMetastoreThriftClient.Builder thriftClientBuilder,
        @Nonnegative int threadPoolSize) {
      this.thriftClientBuilder = Preconditions.checkNotNull(thriftClientBuilder,
          "Thrift client builder was null.");
  ...
  public Handle open(ConnectorArguments arguments) throws Exception {
    ...
    HiveMetastoreThriftClient.Builder thriftClientBuilder = new HiveMetastoreThriftClient.Builder(
        requestedClientVersion)
        .withHost(arguments.getHost())
        .withPort(
            arguments.getPort(Integer.parseInt(ConnectorArguments.OPT_HIVE_METASTORE_PORT_DEFAULT)))
