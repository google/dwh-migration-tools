...
@RespectsInput(order = 100, arg = ConnectorArguments.OPT_HOST, description = "The hostname of the Hive metastore.", defaultValue = ConnectorArguments.OPT_HOST_DEFAULT)
@RespectsInput(order = 101, arg = ConnectorArguments.OPT_PORT, description = "The port of the Hive metastore.", defaultValue = ConnectorArguments.OPT_HIVE_METASTORE_PORT_DEFAULT)
@RespectsInput(order = 400, arg = ConnectorArguments.OPT_HIVE_METASTORE_VERSION, description = "The version of the Hive metastore.", defaultValue = ConnectorArguments.OPT_HIVE_METASTORE_VERSION_DEFAULT)
@RespectsInput(order = 401, arg = ConnectorArguments.OPT_HIVE_METASTORE_DUMP_PARTITION_METADATA, description = "Dump partition metadata; you may wish to disable this for production metastores with a significant number of partitions due to Thrift client performance implications.", defaultValue = ConnectorArguments.OPT_HIVE_METASTORE_DUMP_PARTITION_METADATA_DEFAULT)
@RespectsInput(order = 500, arg = ConnectorArguments.OPT_THREAD_POOL_SIZE, description = "The size of the thread pool to use when dumping table metadata.")
public abstract class AbstractHiveConnector extends AbstractConnector {
    ...
    /**
     * Each thread in the pool is lazily assigned a new Thrift client upon first use;
     * we don't need to replace them between task executions as the Thrift clients do not preserve state
     * between requests. The important point is that a single Thrift client cannot handle multiple
     * requests simultaneously, by keeping each client thread-local we avoid concurrent use.
     */
    @ThreadSafe
    public static class ThriftClientPool implements AutoCloseable {
        ...
        public ThriftClientPool(@Nonnull String name, @Nonnull HiveMetastoreThriftClient.Builder thriftClientBuilder, int threadPoolSize) {
            ...
            this.threadLocalThriftClient = ThreadLocal.withInitial(() -> {
                ...
                try {
                    HiveMetastoreThriftClient client = new HiveMetastoreThriftClient.Builder(thriftClientBuilder).withName(threadName).build();
                    synchronized (lock) {
                        builtClients.add(client);
                    }
                    return client;
                } catch (TTransportException e) {
                    throw new RuntimeException("Unable to build Thrift client '" + threadName + "' owned by pooled client '" + name + "' due to transport exception.", e);
                }
            });
        }
        ...
        public void close() throws Exception {
            ...
            synchronized (lock) {
                for (HiveMetastoreThriftClient client : builtClients) {
                    try {
                        LOG.debug("Closing thread-local Thrift client '{}' owned by pooled client '{}'.", client.getName(), name);
                        client.close();
                    } catch (Exception ioe) {
                        LOG.warn("Unable to close Thrift client '" + client.getName() + "' owned by pooled client '" + name + "'. " + client.getName(), ioe);
                    }
                }
            }
    ...
    protected static class ThriftClientHandle extends AbstractHandle {
        ...
        public ThriftClientHandle(@Nonnull HiveMetastoreThriftClient.Builder thriftClientBuilder, @Nonnegative int threadPoolSize) {
            this.thriftClientBuilder = Preconditions.checkNotNull(thriftClientBuilder, "Thrift client builder was null.");
    ...
    protected static abstract class AbstractHiveTask extends AbstractTask<Void> {
        ...
        @ForOverride
        protected abstract void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle handle) throws Exception;

        @Override
        protected Void doRun(TaskRunContext context, ByteSink sink, @Nonnull Handle handle) throws Exception {
