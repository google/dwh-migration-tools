...
@RespectsInput(order = 0, env = ServiceOptions.CREDENTIAL_ENV_NAME, description = "The path to the Google credentials JSON file.", required = "yes")
public abstract class AbstractBigQueryConnector extends AbstractConnector {
    ...
    public static abstract class AbstractBigQueryTask extends AbstractTask<Void> {
        ...
        // This can return null. Really, we want someone to use BigQueryCallable<@NonNull Foo> to indicate whether their callable can return null.
        protected static <T> T runWithBackOff(@Nonnull BigQueryCallable<T> callable) throws BigQueryException, IOException, InterruptedException {
            ...
            private static class Itr<T> extends UnmodifiableIterator<T> {
                ...
                public boolean hasNext() {
                    for (;;) {
                        ...
                        // However, if we catch it, we can't guarantee that next() won't throw on every call thereafter.
                        return true;
                    }
                }
    ...
    public Handle open(@Nonnull ConnectorArguments arguments) throws Exception {
        // There is currently a bug in the BigQuery client libraries. The documentation says that if we have a
        // file specified in the environment, it will take precedence over the credentials present in the
        // account running the process. This doesn't actually work properly, so we have our own logic here.
        String credentialsFile = System.getenv(ServiceOptions.CREDENTIAL_ENV_NAME);
        BigQuery bigQuery;
        if (credentialsFile != null) {
            bigQuery = BigQueryOptions.newBuilder()
                .setCredentials(GoogleCredentials.fromStream(FileUtils.openInputStream(FileUtils.getFile(credentialsFile))))
                .build()
                .getService();
        } else {
