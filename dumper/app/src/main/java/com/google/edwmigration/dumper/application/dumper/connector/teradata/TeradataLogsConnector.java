...
public class TeradataLogsConnector extends AbstractTeradataConnector implements LogsConnector,
    TeradataLogsDumpFormat {
  ...
  protected static class TeradataLogsJdbcTask extends AbstractJdbcTask<Void> {
    ...
    private String getSql(@Nonnull JdbcHandle handle) {
      Function<String, Boolean> validator = expression -> isValid(handle.getJdbcTemplate(),
          expression);
      Predicate<String> predicate = expression -> state.expressionValidity.computeIfAbsent(
    ...
    /**
     * For each potential expression in EXPRESSIONS, work out whether this Teradata accepts it, and
     * if so, use it as part of the eventual query.
     *
    ...
    private Boolean isValid(@Nonnull JdbcTemplate template, @Nonnull String expression) {
      ...
      try {
        ...
        LOG.info("Attribute '{}' is absent, will use NULL in projection: {}", expression,
            e.getMessage());
  ...
  protected static class TeradataAssessmentLogsJdbcTask extends TeradataLogsJdbcTask {
    ...
    public TeradataAssessmentLogsJdbcTask(@Nonnull String targetPath, SharedState state,
        String logTable, String queryTable, List<String> conditions, ZonedInterval interval,
        List<String> orderBy) {
  ...
  public void addTasksTo(List<? super Task<?>> out, @Nonnull ConnectorArguments arguments)
    ...
    for (ZonedInterval interval : intervals) {
      String file =
          ZIP_ENTRY_PREFIX + DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(interval.getStartUTC())
              + ".csv";
      if (isAssessment) {
        List<String> orderBy = Arrays.asList("ST.QueryID", "ST.SQLRowNo");
        out.add(new TeradataAssessmentLogsJdbcTask(file, state, logTable, queryTable, conditions,
            interval, orderBy)
