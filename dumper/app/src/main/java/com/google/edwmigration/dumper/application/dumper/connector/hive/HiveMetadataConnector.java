...
public class HiveMetadataConnector extends AbstractHiveConnector implements HiveMetadataDumpFormat, MetadataConnector {
    ...
    private static abstract class AbstractHiveMetadataTask extends AbstractHiveTask {
        ...
        private AbstractHiveMetadataTask(String targetPath, @Nonnull Predicate<String> schemaPredicate) {
            super(targetPath);
    ...
    private static class SchemataTask extends AbstractHiveMetadataTask implements SchemataFormat {
        ...
        protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle) throws Exception {
            try (HiveMetastoreThriftClient client = thriftClientHandle.newClient("schemata-task-client");
                 RecordProgressMonitor monitor = new RecordProgressMonitor("Writing database names to " + getTargetPath());
    ...
    private static class TablesJsonTask extends AbstractHiveMetadataTask implements TablesJsonTaskFormat {
        ...
        private TablesJsonTask(@Nonnull Predicate<String> schemaPredicate, boolean isHiveMetastorePartitionMetadataDumpingEnabled) {
            super(ZIP_ENTRY_NAME, schemaPredicate);
        ...
        private void dumpTable(@Nonnull ConcurrentProgressMonitor monitor, @Nonnull Writer writer, @Nonnull ThriftClientPool clientPool, @Nonnull String databaseName, @Nonnull String tableName) {
            clientPool.execute((thriftClient) -> {
                try {
                    ...
                    LOG.warn("Metadata cannot be dumped for table " + databaseName + "." + tableName + " due to an exception, skipping it.", e);
                }
            });
        }
    ...
    private static class FunctionsTask extends AbstractHiveMetadataTask implements FunctionsFormat {
        ...
        protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle) throws Exception {
            try (HiveMetastoreThriftClient client = thriftClientHandle.newClient("functions-task-client");
                 RecordProgressMonitor monitor = new RecordProgressMonitor("Writing functions to " + getTargetPath())) {
