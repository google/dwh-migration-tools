...
public class HiveMetadataConnector extends AbstractHiveConnector implements HiveMetadataDumpFormat,
    MetadataConnector {
    ...
    protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle)
      ...
      try (HiveMetastoreThriftClient client = thriftClientHandle.newClient("schemata-task-client");
          RecordProgressMonitor monitor = new RecordProgressMonitor(
              "Writing database names to " + getTargetPath());
  ...
  private static class DatabasesTask extends AbstractHiveMetadataTask implements DatabasesFormat {
    ...
    protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle)
      ...
      try (HiveMetastoreThriftClient client = thriftClientHandle.newClient("databases-task-client");
          RecordProgressMonitor monitor = new RecordProgressMonitor(
              "Writing databases info to " + getTargetPath());
          CSVPrinter printer = FORMAT.withHeader(Header.class).print(writer)) {
        List<? extends String> allDatabases = client.getAllDatabaseNames();
  ...
  private static class TablesJsonTask extends AbstractHiveMetadataTask implements
      TablesJsonTaskFormat {

    private final boolean isHiveMetastorePartitionMetadataDumpingEnabled;

    private TablesJsonTask(@Nonnull Predicate<String> schemaPredicate,
        boolean isHiveMetastorePartitionMetadataDumpingEnabled) {
      super(ZIP_ENTRY_NAME, schemaPredicate);
      this.isHiveMetastorePartitionMetadataDumpingEnabled = isHiveMetastorePartitionMetadataDumpingEnabled;
    }

    @Override
    protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle)
        throws Exception {
      try (ThriftClientPool clientPool = thriftClientHandle.newMultiThreadedThriftClientPool(
          "tables-task-pooled-client")) {
        clientPool.execute(thriftClient -> {
          ...
          for (String databaseName : allDatabases) {
            if (isIncludedSchema(databaseName)) {
              List<? extends String> allTables = thriftClient.getAllTableNamesInDatabase(
                  databaseName);
    ...
    private void dumpTable(@Nonnull ConcurrentProgressMonitor monitor, @Nonnull Writer writer,
        @Nonnull ThriftClientPool clientPool, @Nonnull String databaseName,
      ...
      clientPool.execute((thriftClient) -> {
        try {
          ...
          for (PartitionKey partitionKey : table.getPartitionKeys()) {
            TableMetadata.PartitionKeyMetadata partitionKeyMetadata = new TableMetadata.PartitionKeyMetadata();
          ...
          if (isHiveMetastorePartitionMetadataDumpingEnabled) {
            ...
            // This call to the Thrift client is particularly expensive when many partitions are present.
          ...
          // Failure to dump a single table should not prevent the rest of the tables from being dumped.
          LOG.warn("Metadata cannot be dumped for table " + databaseName + "." + tableName
              + " due to an exception, skipping it.", e);
        }
      });
    }
  ...
  private static class FunctionsTask extends AbstractHiveMetadataTask implements FunctionsFormat {
    ...
    protected void run(@Nonnull Writer writer, @Nonnull ThriftClientHandle thriftClientHandle)
        throws Exception {
      try (HiveMetastoreThriftClient client = thriftClientHandle.newClient("functions-task-client");
          RecordProgressMonitor monitor = new RecordProgressMonitor(
              "Writing functions to " + getTargetPath())) {
