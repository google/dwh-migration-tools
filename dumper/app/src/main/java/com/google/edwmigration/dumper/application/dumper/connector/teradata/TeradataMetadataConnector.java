...
public class TeradataMetadataConnector extends AbstractTeradataConnector implements
    MetadataConnector, TeradataMetadataDumpFormat {
  ...
  public void addTasksTo(List<? super Task<?>> out, ConnectorArguments arguments) {
    ...
    String whereDatabaseNameClause = new SqlBuilder().withWhereInVals("\"DatabaseName\"",
        arguments.getDatabases()).toWhereClause();
    String whereDataBaseNameClause = new SqlBuilder().withWhereInVals("\"DataBaseName\"",
        arguments.getDatabases()).toWhereClause();
    String whereBDatabaseNameClause = new SqlBuilder().withWhereInVals("\"B_DatabaseName\"",
        arguments.getDatabases()).toWhereClause();
    String whereChildDBClause = new SqlBuilder().withWhereInVals("\"ChildDB\"",
        arguments.getDatabases()).toWhereClause();
    String whereParentDBClause = new SqlBuilder().withWhereInVals("\"ParentDB\"",
        arguments.getDatabases()).toWhereClause();
    ...
    if (arguments.isAssessment()) {
      ...
      out.add(new TeradataJdbcSelectTask(TableSizeVFormat.ZIP_ENTRY_NAME,
          TaskCategory.OPTIONAL,
          "SELECT %s FROM DBC.TableSizeV" + whereDataBaseNameClause
              + " ;")); // What's a VProc? (AMP, col 1)
