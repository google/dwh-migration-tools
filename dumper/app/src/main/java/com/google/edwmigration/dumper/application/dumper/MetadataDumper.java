...
public class MetadataDumper {
  ...
  public void run(@Nonnull String... args) throws Exception {
    ...
    if (connector == null) {
      LOG.error(
          "Target DBMS " + connectorName + " not supported; available are " + CONNECTORS.keySet()
              + ".");
      return;
    }
  ...
  @CheckForNull
  private <T> T runTask(@Nonnull TaskRunContext context, @Nonnull TaskSetState.Impl state,
      Task<T> task) throws MetadataDumperUsageException {
    try {
      CHECK:
      {
        TaskState ts = state.getTaskState(task);
        Preconditions.checkState(ts == TaskState.NOT_STARTED,
            "TaskState was bad: " + ts + " for " + task);
      }
      ...
      try (Closer closer = Closer.create()) {
        ...
        if (StringUtils.endsWithIgnoreCase(outputFile.getName(), ".zip")) {
          ...
          FileSystem fileSystem = closer.register(
              FileSystems.newFileSystem(outputUri, fileSystemProperties));
          sinkFactory = new FileSystemOutputHandleFactory(fileSystem,
              "/");   // It's required to be "/"
        } else {
          if (outputFile.exists()) {
        ...
        TaskRunContext runContext = new TaskRunContext(sinkFactory, handle,
            arguments.getThreadPoolSize()) {
          @Override
