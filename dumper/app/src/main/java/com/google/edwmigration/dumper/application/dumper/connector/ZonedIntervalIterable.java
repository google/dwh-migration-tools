...
public class ZonedIntervalIterable implements Iterable<ZonedInterval> {
  ...
  @Nonnull
  @VisibleForTesting
  /* pp */ static ZonedIntervalIterable forDateTimeRange(@Nonnull ZonedDateTime start,
      @Nonnull ZonedDateTime end, @Nonnull ChronoUnit unit) {
    return new ZonedIntervalIterable(start, end, unit);
  }

  @Nonnull
  @VisibleForTesting
  /* pp */ static ZonedIntervalIterable forTimeUnitsUntil(@Nonnull ZonedDateTime now,
      @Nonnegative int unitCount, @Nonnull ChronoUnit unit) {
    return forDateTimeRange(
        now.minus(unitCount, unit),
        now.plus(1, unit),
        // view comment in ZonedIntervalIterableTest, adding .minus(1, ChronoUnit.MILLIS) only fixes cases when now is padded at xx:00:00
        unit
    );
  }
  ...
  @Nonnull
  @VisibleForTesting
  /* pp */ static ZonedIntervalIterable forTimeUnitsUntilNow(@Nonnegative int unitCount,
  ...
  public static ZonedIntervalIterable forConnectorArguments(@Nonnull ConnectorArguments arguments)
    ...
    if (arguments.getQueryLogStart() != null || arguments.getQueryLogEnd() != null) {
      ...
      if (arguments.getQueryLogEnd() == null) {
        LOG.info("Missing option --query-log-end will be defaulted to: "
            + arguments.getQueryLogEndOrDefault());
      }

      LOG.info("Log entries from {} to {} will be exported in increments of 1 hour.",
          arguments.getQueryLogStart(), arguments.getQueryLogEndOrDefault());
      return ZonedIntervalIterable.forDateTimeRange(arguments.getQueryLogStart(),
          arguments.getQueryLogEndOrDefault(), ChronoUnit.HOURS);
    }
    ...
    LOG.info("Log entries within the last {} days will be exported in increments of 1 hour.",
        daysToExport);
    return ZonedIntervalIterable.forTimeUnitsUntilNow(24 * daysToExport, ChronoUnit.HOURS);

  }

  private ZonedIntervalIterable(@Nonnull ZonedDateTime queryLogStart,
      @Nonnull ZonedDateTime queryLogEnd, @Nonnull TemporalUnit unit) {
    ...
    Preconditions.checkState(queryLogStart.isBefore(queryLogEnd),
        "Start date %s must precede end date %s", queryLogStart, queryLogEnd);
