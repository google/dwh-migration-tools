...
public class ZonedIntervalIterable implements Iterable<ZonedInterval> {
    ...
    @Nonnull
    @VisibleForTesting
    /* pp */ static ZonedIntervalIterable forDateTimeRange(@Nonnull ZonedDateTime start, @Nonnull ZonedDateTime end, @Nonnull ChronoUnit unit) {
        return new ZonedIntervalIterable(start, end, unit);
    }

    @Nonnull
    @VisibleForTesting
    /* pp */ static ZonedIntervalIterable forTimeUnitsUntil(@Nonnull ZonedDateTime now, @Nonnegative int unitCount, @Nonnull ChronoUnit unit) {
        return forDateTimeRange(
                now.minus(unitCount, unit),
                now.plus(1, unit), // view comment in ZonedIntervalIterableTest, adding .minus(1, ChronoUnit.MILLIS) only fixes cases when now is padded at xx:00:00
                unit
        );
    }
    ...
    @Nonnull
    @VisibleForTesting
    /* pp */ static ZonedIntervalIterable forTimeUnitsUntilNow(@Nonnegative int unitCount, @Nonnull ChronoUnit unit) {
        return forTimeUnitsUntil(ZonedDateTime.now(ZoneOffset.UTC), unitCount, unit);
    }

    /**
     * Builds a ZonedIntervalIterable from connector arguments, the intervals will all be one hour long (ChronoUnit.HOURS) and have and inclusive
     * starting datetime and exclusive ending datetime (i.e.: start <= t < end ).
     *
     ...
     */
    @Nonnull
    public static ZonedIntervalIterable forConnectorArguments(@Nonnull ConnectorArguments arguments) throws MetadataDumperUsageException {

        if (arguments.getQueryLogStart() != null || arguments.getQueryLogEnd() != null) {
            if (arguments.getQueryLogDays() != null)
                throw new MetadataDumperUsageException("Incompatible options, either specify a number of log days to export or a start/end timestamp.");

            if (arguments.getQueryLogStart() == null)
                throw new MetadataDumperUsageException("Missing option --query-log-start must be specified.");
            if (arguments.getQueryLogEnd() == null)
                LOG.info("Missing option --query-log-end will be defaulted to: " + arguments.getQueryLogEndOrDefault());
    ...
    private ZonedIntervalIterable(@Nonnull ZonedDateTime queryLogStart, @Nonnull ZonedDateTime queryLogEnd, @Nonnull TemporalUnit unit) {
