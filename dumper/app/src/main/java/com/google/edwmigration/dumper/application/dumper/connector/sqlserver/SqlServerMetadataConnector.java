 ...
 *  * --connector sqlserver --driver /path/to/mssql-jdbc.jar --host codetestserver.database.windows.net --database CodeTestDataWarehouse --user cw --password password
 *
 * @author swapnil
 */
...
@RespectsInput(order = ConnectorArguments.OPT_PORT_ORDER, arg = ConnectorArguments.OPT_PORT, description = "The port of the server.", required = ConnectorArguments.OPT_REQUIRED_IF_NOT_URL, defaultValue = "" + SqlServerMetadataConnector.OPT_PORT_DEFAULT)
@RespectsArgumentUser
@RespectsArgumentPassword
@RespectsArgumentDriver
@RespectsArgumentDatabaseForConnection
@RespectsArgumentUri
public class SqlServerMetadataConnector extends AbstractJdbcConnector implements MetadataConnector, SqlServerMetadataDumpFormat {
    ...
    private static final String SYSTEM_SCHEMAS = "('sys', 'information_schema', 'performance_schema')";
    ...
    public Handle open(ConnectorArguments arguments) throws Exception {
        ...
        if (url == null) {
            ...
            if (!databases.isEmpty())
                buf.append(";database=").append(databases.get(0));
            url = buf.toString();
        }
