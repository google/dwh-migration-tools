  ...
  private void checkArguments(@Nonnull ConnectorArguments arguments)
    ...
    if (this.multiMode) {
      if (arguments.getQueryLogStart()
          == null)  // no check for --query-log-end because it will be defaulted by ZonedIntervalIterable if absent
      {
    ...
    if (!errors.isEmpty()) {
      throw new MetadataDumperUsageException("Missing arguments for connector generic-args :",
          errors);
    }
