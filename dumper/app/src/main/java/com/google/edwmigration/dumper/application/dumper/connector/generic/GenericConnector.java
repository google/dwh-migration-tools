...
/**
 *
 * there is no generics-logs and generic-meta .. it's the same
 *   if start & end dates are given, it generates multiple queries and is like 'logs' mode.
 */
...
public class GenericConnector extends AbstractJdbcConnector implements LogsConnector {
    ...
    private static final DateTimeFormatter SQL_FORMAT = DateTimeFormatter.ISO_OFFSET_DATE_TIME.withZone(ZoneOffset.UTC);
    ...
    private void checkArguments(@Nonnull ConnectorArguments arguments) throws MetadataDumperUsageException {
        ...
        if (this.uri == null)
            errors.add("--url is required for generic-logs connector");
        this.driverPaths = arguments.getDriverPaths();
        this.driverClass = arguments.getDriverClass();
        if (this.driverClass == null)
            errors.add("--driver is required for generic-logs connector");

        this.query = arguments.getGenericQuery();
        if (this.query == null)
            errors.add("--generic-query is required for generic-logs connector");
        ...
        if (this.multiMode) {
            if (arguments.getQueryLogStart() == null)  // no check for --query-log-end because it will be defaulted by ZonedIntervalIterable if absent
                errors.add("Missing option --query-log-start must be specified.");
            if (this.entryName == null)
                this.entryName = "generic-logs-{period}.csv";
            if (this.query != null) {
            ...
            if (this.entryName == null)
                this.entryName = "generic-query.csv";
        }

        if (!errors.isEmpty())
            throw new MetadataDumperUsageException("Missing arguments for connector generic-args :", errors);
