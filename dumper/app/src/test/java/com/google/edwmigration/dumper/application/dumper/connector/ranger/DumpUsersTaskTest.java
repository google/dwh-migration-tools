...
public class DumpUsersTaskTest extends AbstractRangerTaskTest {

  @Test
  public void doRun_success() throws Exception {
    DumpUsersTask task = new DumpUsersTask();
    mockSuccessfulResponseFromResource("ranger/users/api-response.json");

    task.doRun(/* context= */ null, sink, handle);

    String actual = sink.openStream().toString();
    String expected = RangerTestResources.getResourceAsString("ranger/users/expected-output.jsonl");
