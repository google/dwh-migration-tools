...
public class TeradataMetadataConnectorTest extends AbstractConnectorExecutionTest implements
    TeradataMetadataDumpFormat {
  ...
  public void testConnectorSynthetic() throws Exception {
    ...
    File outputFile = runDumper("teradata-simulation-schema.sql", HugeDatabase.N_DB,
        HugeDatabase.N_TBL, HugeDatabase.N_COL);
  ...
  public void testConnectorSyntheticSmall() throws Exception {
    ...
    File outputFile = runDumper("teradata-simulation-schema.sql", SmallDatabase.N_DB,
        SmallDatabase.N_TBL, SmallDatabase.N_COL);
  ...
  private File runDumper(String schemaSql, final int nDb, final int nTbl, final int nCol)
    ...
    CONTINUE:
    {
      ...
      dumper.run(args.toArray(ArrayUtils.EMPTY_STRING_ARRAY));
      assertTrue("Second run of dumper was too slow.", stopwatch.elapsed(TimeUnit.SECONDS)
