...
/**
 *
 * ./gradlew :compilerworks-application-dumper:test --tests TeradataMetadataConnectorTest.testConnectorSynthetic -Dtest.verbose=true -Dtest-sys-prop.test.dumper=true
 */
@RunWith(JUnit4.class)
public class TeradataMetadataConnectorTest extends AbstractConnectorExecutionTest implements TeradataMetadataDumpFormat {
    ...
    private static boolean exec(@Nonnull PreparedStatement statement, Object... arguments) throws SQLException {
    ...
    public void testConnectorSynthetic() throws Exception {
        ...
        File outputFile = runDumper("teradata-simulation-schema.sql", HugeDatabase.N_DB, HugeDatabase.N_TBL, HugeDatabase.N_COL);
        ...
        validator.withEntryValidator(TableSizeVFormat.ZIP_ENTRY_NAME, TableSizeVFormat.Header.class)
    ...
    public void testConnectorSyntheticSmall() throws Exception {
        ...
        File outputFile = runDumper("teradata-simulation-schema.sql", SmallDatabase.N_DB, SmallDatabase.N_TBL, SmallDatabase.N_COL);
    ...
    @Nullable
    private File runDumper(String schemaSql, final int nDb, final int nTbl, final int nCol) throws Exception {
        if (true) {
            ...
            try (ProgressMonitor monitor = new RecordProgressMonitor("Creating sample schema", nDb * nTbl * nCol).withBlockSize(256)) {
                jdbcTemplate.execute(new ConnectionCallback<Void>() {
                    @Override
                    public Void doInConnection(Connection connection) throws SQLException, DataAccessException {
                        PreparedStatement q_db = connection.prepareStatement("insert into DBC.DBase (\"DatabaseName\", \"DatabaseId\") values (?, ?)");
                        PreparedStatement q_tbl = connection.prepareStatement("insert into DBC.TVM (\"DatabaseId\", \"TVMNameI\", \"TVMName\", \"TVMId\", \"TableKind\") values (?, ?, ?, ?, ?)");
                        PreparedStatement q_col = connection.prepareStatement("insert into DBC.TVFields (\"TableId\", \"FieldName\", \"FieldId\", \"FieldType\") values (?, ?, ?, ?)");
        ...
        CONTINUE:
        {
            ...
            dumper.run(args.toArray(ArrayUtils.EMPTY_STRING_ARRAY));
            assertTrue("Second run of dumper was too slow.", stopwatch.elapsed(TimeUnit.SECONDS) < 10);   // The main dump takes about 2 minutes. This noop should take 1 second.
        }
        return outputFile;
    }
}
