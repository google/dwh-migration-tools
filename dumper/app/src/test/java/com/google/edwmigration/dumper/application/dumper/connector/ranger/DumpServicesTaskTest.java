...
public class DumpServicesTaskTest extends AbstractRangerTaskTest {

  @Test
  public void doRun_success() throws Exception {
    DumpServicesTask task = new DumpServicesTask();
    mockSuccessfulResponseFromResource("ranger/services/api-response.json");

    task.doRun(/* context= */ null, sink, handle);

    String actual = sink.openStream().toString();
    String expected =
        RangerTestResources.getResourceAsString("ranger/services/expected-output.jsonl");
