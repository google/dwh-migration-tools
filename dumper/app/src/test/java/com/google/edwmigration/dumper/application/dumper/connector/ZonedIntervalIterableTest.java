...
public class ZonedIntervalIterableTest {
    ...
    ConnectorArguments.ZonedParser zonedParserStart = new ConnectorArguments.ZonedParser(ConnectorArguments.ZonedParser.DEFAULT_PATTERN, ConnectorArguments.ZonedParser.DayOffset.START_OF_DAY);
    ConnectorArguments.ZonedParser zonedParserEnd = new ConnectorArguments.ZonedParser(ConnectorArguments.ZonedParser.DEFAULT_PATTERN, ConnectorArguments.ZonedParser.DayOffset.END_OF_DAY);
    ...
    /**
     * Check that every interval length is one millisecond shorter than temporal unit and that
     * the start of interval is truncated to unit, i.e.: interval = [start, start + 1 unit)
     */
    ...
    // Interval is 2019-11-02T00:00-07:00[America/Los_Angeles]...2019-11-02T23:59:59.999-07:00[America/Los_Angeles]
    // Interval is 2019-11-03T00:00-07:00[America/Los_Angeles]...2019-11-03T23:59:59.999-08:00[America/Los_Angeles]
    // Interval is 2019-11-04T00:00-08:00[America/Los_Angeles]...2019-11-04T23:59:59.999-08:00[America/Los_Angeles]
    // Interval is 2019-11-05T00:00-08:00[America/Los_Angeles]...2019-11-05T23:59:59.999-08:00[America/Los_Angeles]

    private void assertIterations(int expected, @Nonnull String from, @Nonnull String to, ChronoUnit chronoUnit) {
        ZonedIntervalIterable iterable = ZonedIntervalIterable.forDateTimeRange(
    ...
    public void testHours() {
        ...
        testIterable(2, ZonedIntervalIterable.forTimeUnitsUntil(ZonedDateTime.of(2020, 1, 1, 0, 0, 0, 0, ZoneId.of("UTC")), 1, ChronoUnit.HOURS));
        ...
        // will - in my opinion - lead to double counting almost anytime, if I dump last 1 day every midnight
        // I will get 24 full hours + 1 partial hour, the partial hour will be included in the first slice of
        // next dump, assuming nightly dumps at midnight + some seconds/minutes
        int daysToExport = 1;
        testIterable(25, ZonedIntervalIterable.forTimeUnitsUntilNow(24 * daysToExport, ChronoUnit.HOURS));
    ...
    public void testForLogStartAndLogEnd() throws Throwable {
        ...
        ConnectorArguments arguments = new ConnectorArguments(new String[] {
                "--query-log-start", requestedStart.toString() , "--query-log-end",  requestedEnd.toString(), "--connector", "foobar"
        });
    ...
    public void testForLogStartTimeAndLogEndTime() throws Throwable {
        ...
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(ConnectorArguments.ZonedParser.DEFAULT_PATTERN, Locale.US);
        ...
        ZonedDateTime expectedStart = requestedStartParsed.atZone(ZoneOffset.UTC).truncatedTo(ChronoUnit.HOURS);
        ZonedDateTime expectedEnd = requestedEndParsed.atZone(ZoneOffset.UTC).truncatedTo(ChronoUnit.HOURS);

        ConnectorArguments arguments = new ConnectorArguments(new String[] {
                "--query-log-start", requestedStart, "--query-log-end",  requestedEnd, "--connector", "foobar"
        });
        checkIntervalForArguments(expectedStart, expectedEnd, arguments);
    }
    ...
    public void testForLogDays() throws Throwable {
        ...
        ZonedDateTime expectedStart = nowAtUTC.truncatedTo(ChronoUnit.HOURS).minusDays(daysExpected).truncatedTo(ChronoUnit.HOURS);
        ZonedDateTime expectedEnd = nowAtUTC.truncatedTo(ChronoUnit.HOURS).plusHours(1).truncatedTo(ChronoUnit.HOURS);
        ConnectorArguments arguments = new ConnectorArguments(new String[] {
                "--query-log-days", "" + daysExpected , "--connector", "foobar"
        });
        checkIntervalForArguments(expectedStart, expectedEnd, arguments);
    }
    ...
    private void checkIntervalForArguments(ZonedDateTime expectedStart, ZonedDateTime expectedEnd, ConnectorArguments arguments) throws MetadataDumperUsageException {
