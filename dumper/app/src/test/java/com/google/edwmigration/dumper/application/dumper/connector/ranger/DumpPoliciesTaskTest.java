...
public class DumpPoliciesTaskTest extends AbstractRangerTaskTest {

  @Test
  public void doRun_success() throws Exception {
    DumpPoliciesTask task = new DumpPoliciesTask();
    mockSuccessfulResponseFromResource("ranger/policies/api-response.json");

    task.doRun(/* context= */ null, sink, handle);

    String actual = sink.openStream().toString();
    String expected =
        RangerTestResources.getResourceAsString("ranger/policies/expected-output.jsonl");
