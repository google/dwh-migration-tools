...
public class HiveTestSchemaBuilder {
  ...
  public static List<List<String>> getStatements(int dbCount, int tblCount, int colCount,
      int pCount) {
  ...
  public static List<String> createDatabases(int dbCount) {
    return IntStream.range(0, dbCount).mapToObj(HiveTestSchemaBuilder::createDatabase)
        .collect(Collectors.toList());
  }

  public static List<String> createPartitions(int dbIndex, int tblIndex, int partitionCount,
      int partitionSplit) {
    ...
    return Lists.partition(partitions, partitionSplit).stream()
        ...
        .map(p -> String.format("alter table %s%d.%s%d add %s", DATABASE_PREFIX, dbIndex,
            TABLE_PREFIX, tblIndex, p))
        .collect(Collectors.toList());
  }
  ...
  public static String createTable(int dbIndex, int tblIndex, int colCount) {
    return String.format("create table %s%d.%s%d (%s) partitioned by (%s)",
        DATABASE_PREFIX, dbIndex, TABLE_PREFIX, tblIndex, getColumns(colCount),
