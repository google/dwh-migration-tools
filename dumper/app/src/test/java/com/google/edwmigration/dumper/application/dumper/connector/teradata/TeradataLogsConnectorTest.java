...
public class TeradataLogsConnectorTest extends AbstractConnectorExecutionTest {
  ...
  public void testExecution() throws Exception {
    ...
    try (JdbcHandle handle = DumperTestUtils.newJdbcHandle(dbFile);
      ...
      handle
          ...
          .execute(
              "create table "
                  + TeradataSqlQueryFactoryBuilder.DEF_LOG_TABLE
              ...
              public Void doInConnection(Connection connection)
                ...
                PreparedStatement dbqlogtbl =
                    connection.prepareStatement(
                        "insert into DBC.DBQLogTbl (\"procid\", \"collecttimestamp\", \"username\","
                            + " \"errorcode\", \"querytext\", \"queryid\") values (?, ?, ?, ?, ?,"
                            + " ?)");
                PreparedStatement dbqlsqltbl =
                    connection.prepareStatement(
                        "insert into DBC.DBQLSqlTbl (\"procid\", \"collecttimestamp\", \"queryid\","
                            + " \"sqlrowno\", \"sqltextinfo\") values (?, ?, ?, ?, ?)");
  ...
  public void testHeaderAndColumns() {
    checkNames(TeradataLogsDumpFormat.Header.values(),
        TeradataSqlQueryFactoryBuilder.EXPRESSIONS);
    checkNames(TeradataLogsDumpFormat.HeaderLSql.values(),
        TeradataSqlQueryFactoryBuilder.EXPRESSIONS_LSQL_TBL.toArray(new String[0]));
    checkNames(TeradataLogsDumpFormat.HeaderLog.values(),
        TeradataSqlQueryFactoryBuilder.EXPRESSIONS_LOG_TBL.toArray(new String[0]));
  }
}
