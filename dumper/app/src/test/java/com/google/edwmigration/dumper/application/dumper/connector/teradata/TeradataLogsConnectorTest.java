...
public class TeradataLogsConnectorTest extends AbstractConnectorExecutionTest {
  ...
  public void testExecution() throws Exception {
    ...
    try (JdbcHandle handle = DumperTestUtils.newJdbcHandle(dbFile);
        FileSystem fileSystem = FileSystems.newFileSystem(outputUri,
      ...
      handle.getJdbcTemplate().execute("create table " + TeradataLogsConnector.DEF_LOG_TABLE
          + " (UserName varchar, errorcode int, StartTime int)");

      TaskRunContext runContext = new DummyTaskRunContext(sinkFactory, handle);
      List<Task<?>> tasks = new ArrayList<>();
      connector.addTasksTo(tasks, new ConnectorArguments(
  ...
  public void testConnectorSynthetic() throws Exception {
    ...
    if (false) {
      ...
      try (Connection connection = dataSource.getConnection()) {
        URL url = Resources.getResource(ResourceLocation.class,
            "teradata-simulation-schema-query-logs.sql");
        ScriptUtils.executeSqlScript(connection, new UrlResource(url));
      }
      JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);

      LOG.debug("Rebuilding database....");
      try (ProgressMonitor monitor = new RecordProgressMonitor("Creating sample schema",
          N_QUERY_LOGS).withBlockSize(256)) {
        jdbcTemplate.execute(new ConnectionCallback<Void>() {
          @Override
          public Void doInConnection(Connection connection)
              throws SQLException, DataAccessException {
            PreparedStatement dbqlogtbl = connection.prepareStatement(
                "insert into DBC.DBQLogTbl (\"procid\", \"collecttimestamp\", \"username\", \"errorcode\", \"querytext\", \"queryid\") values (?, ?, ?, ?, ?, ?)");
            PreparedStatement dbqlsqltbl = connection.prepareStatement(
            ...
            for (int i_log = 0; i_log < N_QUERY_LOGS; i_log++) {
              int procId = i_log % 10;
              Timestamp collectTimeStamp = new Timestamp(
                  startTime - i_log); // decrement by one millisecond
              final String fullQueryText =
                  "select " + Strings.repeat("'" + i_log + "', ", 50) + "'end';";
              final String abbrevQueryText = fullQueryText.substring(0,
                  100);  // not actual TD behavior, but conveys the point
              exec(dbqlogtbl, procId, collectTimeStamp, newUserName(i_log), errorCode,
                  abbrevQueryText, i_log);
