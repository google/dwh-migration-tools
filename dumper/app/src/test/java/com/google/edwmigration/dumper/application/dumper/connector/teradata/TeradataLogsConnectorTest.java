...
public class TeradataLogsConnectorTest extends AbstractConnectorExecutionTest {
    ...
    public void testExecution() throws Exception {
        ...
        try (JdbcHandle handle = DumperTestUtils.newJdbcHandle(dbFile);
                FileSystem fileSystem = FileSystems.newFileSystem(outputUri, ImmutableMap.of("create", "true"))) {
            ...
            connector.addTasksTo(tasks, new ConnectorArguments(new String[]{"--connector", connector.getName(), "--query-log-days", "1"}));
            for (Task<?> task : tasks) {
                task.run(runContext);
            }
        }
    ...
    public void testConnectorSynthetic() throws Exception {
        ...
        if (false) {
            ...
            try (Connection connection = dataSource.getConnection()) {
                URL url = Resources.getResource(ResourceLocation.class, "teradata-simulation-schema-query-logs.sql");
                ScriptUtils.executeSqlScript(connection, new UrlResource(url));
            }
            JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);

            LOG.debug("Rebuilding database....");
            try (ProgressMonitor monitor = new RecordProgressMonitor("Creating sample schema", N_QUERY_LOGS).withBlockSize(256)) {
                jdbcTemplate.execute(new ConnectionCallback<Void>() {
                    @Override
                    public Void doInConnection(Connection connection) throws SQLException, DataAccessException {
                        PreparedStatement dbqlogtbl = connection.prepareStatement("insert into DBC.DBQLogTbl (\"procid\", \"collecttimestamp\", \"username\", \"errorcode\", \"querytext\", \"queryid\") values (?, ?, ?, ?, ?, ?)");
                        PreparedStatement dbqlsqltbl = connection.prepareStatement("insert into DBC.DBQLSqlTbl (\"procid\", \"collecttimestamp\", \"queryid\", \"sqlrowno\", \"sqltextinfo\") values (?, ?, ?, ?, ?)");
                        ...
                        for (int i_log = 0; i_log < N_QUERY_LOGS; i_log++) {
                            int procId = i_log % 10;
                            Timestamp collectTimeStamp = new Timestamp(startTime - i_log); // decrement by one millisecond
                            final String fullQueryText = "select " + Strings.repeat("'" + i_log + "', ", 50) + "'end';";
                            final String abbrevQueryText = fullQueryText.substring(0, 100);  // not actual TD behavior, but conveys the point
                            exec(dbqlogtbl, procId, collectTimeStamp, newUserName(i_log), errorCode, abbrevQueryText, i_log);
    ...
    private void checkNames(Enum<?>[] vals, String[] exprs) {
        ...
        for (int j = 0; j < vals.length; j++)
            Assert.assertTrue(exprs[j].endsWith(vals[j].name()));
    }
    ...
    public void testHeaderAndColumns() {
        ...
        checkNames(TeradataLogsDumpFormat.HeaderLSql.values(),
                Teradata14LogsConnector.EXPRESSIONS_LSQL_TBL.toArray(new String[0]));
        checkNames(TeradataLogsDumpFormat.HeaderLog.values(),
                Teradata14LogsConnector.EXPRESSIONS_LOG_TBL.toArray(new String[0]));
    }
}
