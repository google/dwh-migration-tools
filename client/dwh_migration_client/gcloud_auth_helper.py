...
"""Checks the status of GCP login and authentication through the gcloud CLI."""
...
def validate_gcloud_auth_settings(project_number: str) -> None:
    ...
    gcloud CLI command. If validation fails, the method will try to log in and
    generate an application-default-authentication.json file.
...
class GcloudAuthHelper:
    """A simpler helper class to check the status of GCP login and authentication
    ...
    def validate_login_status(self) -> None:
        ...
        If validation fails, the method will try to log in through the
        ...
        if "No credentialed accounts" in result:
            ...
            print(
                "Please open the following link in a browser and grant permission "
                "to login..."
            )
