...
"""Manages Batch SQL Translation  using bigquery_migration_v2 python client library."""
...
class BatchSqlTranslator:
    ...
    def __init__(
        self,
        config: config_parser.TranslationConfig,
        preprocessor: Optional[MacroProcessor],
    ) -> None:
        self.config = config
        self.client = bigquery_migration_v2.MigrationServiceClient()
        self.gcs_path: Optional[str] = None
    ...
    # The name of a hidden directory that stores temporary processed files if
    # user defines a macro replacement map. This directory will be deleted
    # (by default) after the job finishes.
    __TMP_DIR_NAME = ".tmp_processed"

    def start_translation(self) -> None:
        """Waits until the workflow finishes by calling the Migration Service
        API every 5 seconds.
        ...
        print(
            "Finished post-processing. The output query files are in %s"
    ...
    def __get_ui_link(self) -> str:
        ...
        return (
            "https://console.cloud.google.com/bigquery/migrations"
            "/offline-translation?projectnumber=%s" % self.config.project_number
        )
    ...
    def __wait_until_job_finished(
        ...
        while processing_seconds < length_seconds:
            ...
            if job_status.state in self.__JOB_FINISHED_STATES:
                return
        print(
            "The job is still running after %d seconds. Please go to the UI page "
            "and download the outputs manually %s"
            % (processing_seconds, self.__get_ui_link())
        )
        sys.exit()

    def list_migration_workflows(self, num_jobs: int = 5) -> None:
        """Lists the most recent bigquery migration workflows status and prints
        on the terminal.
    ...
    def get_migration_workflow(
        self, job_name: str
    ) -> bigquery_migration_v2.types.MigrationWorkflow:
        """Starts a get API call for a migration workflow and print out the
        status on terminal."""
