...
def download_directories(local_dir_map: OrderedDict[str,str], bucket_name: str, gcs_path: str) -> None:
    ...
    for blob in blobs:
        file_name = basename(blob.name)
        sub_dir = blob.name[len(gcs_path) + 1 : -len(file_name)]
        # determine local_dir based on what dir belongs to
        local_dir = ""
        for source_dir in local_dir_map.keys():
            # source_sub_dir = source_dir[-len(sub_dir) + 1:]
            # print("source sub directory: " + source_sub_dir)
            ...
            if Path("/"+ sub_dir).is_relative_to(source_dir):
                if local_target is not None and local_target and local_target.strip():
                    ...
                    # Clean up sub_dir so it is no longer with an abs path 
        ...
        if local_dir is None:
            logging.info('Skipping downloading "%s" because no output directory was selected.', file_name)
            continue
        if not local_dir:
    ...
    # Only want to output files that aren't in sub dir to the first directory in the list