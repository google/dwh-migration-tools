...
public class HdfsToGcsIamBindingMapper extends AbstractHdfsToIamBindingMapper {
  HdfsToGcsIamBindingMapper(
  ...
  protected IamBinding.Builder mapTableToIamBinding(IamBinding.Builder builder, Table table) {
    return builder.resourceType(ResourceType.GCS_MANAGED_FOLDER).resourcePath(table.gcsPath());
  }

  @Override
  protected ImmutableMap<String, Object> mapTableToDefaultObjectMap(Table table) {
    return ImmutableMap.of(
        "resourceType", ResourceType.GCS_MANAGED_FOLDER, "resourcePath", table.gcsPath());
  }
}
